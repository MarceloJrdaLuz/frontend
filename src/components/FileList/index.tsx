...
const FileList = () => {
  const { uploadedFiles: files, handlePdfShow, confirmDelete } = useFiles();
      ...
      {files.map((uploadedFile: IFile) => (
        <li key={uploadedFile.id}>
          <FileInfo>
            ...
            <div>
              <strong onClick={()=> handlePdfShow(uploadedFile.name)}>{uploadedFile.name}</strong>
              <span>
                {uploadedFile.readableSize}{" "}
                {!!uploadedFile.url && (
                  <button onClick={(e) => confirmDelete(uploadedFile.id, uploadedFile.name)}>
                    Excluir
                  </button>
                )}
              </span>
